...
1. From hekate's `Home` menu, navigate to `emuMMC` > `Create emuMMC` > `SD Partition` > `Part X` (`X` being the option that is not greyed out) and wait for Hekate to complete creating the emuMMC.
1. Close the emuMMC creation menu by navigating to the `Close` button, then navigate to `Change emuMMC` > `SD RAW X` (`X` being the option that is not greyed out) and press the `Close` button twice in the top right to return to hekate's `Home` menu.
